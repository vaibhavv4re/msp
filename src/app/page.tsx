  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-background">
      <div className="p-8 max-w-md w-full text-center">
        <h1 className="text-5xl font-serif text-foreground mb-4 tracking-tighter">
          MSP
        </h1>
        <p className="text-muted font-sans text-sm uppercase tracking-widest mb-12">
          Management for Creatives
        </p>

        <button
          onClick={handleGoogleLogin}
          className="w-full bg-foreground text-background py-4 px-8 rounded-full text-[11px] font-sans font-black uppercase tracking-[0.2em] hover:bg-black transition-all shadow-xl shadow-foreground/5"
        >
          Sign in with Google
        </button>

        <p className="mt-12 text-[10px] text-muted uppercase tracking-widest leading-relaxed">
          Secure editorial-grade workbench.<br />
...
function App() {
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      {/* Desktop Navigation */}
      <nav className="hidden md:block w-full sticky top-0 z-50 bg-background/80 backdrop-blur-md border-b border-border-subtle">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          <div className="flex items-center gap-12">
            <h1 className="text-2xl font-serif text-foreground tracking-tighter cursor-pointer" onClick={() => setView("dashboard")}>
              MSP
            </h1>
            ...
            <div className="flex items-center gap-8">
              {[
                ...
                <button
                  ...
                  className={`text-[10px] font-sans font-black uppercase tracking-[0.2em] transition-all hover:text-foreground ${view === item.id ? "text-foreground border-b border-foreground" : "text-muted"
                    }`}
                >
                  {item.label}
                </button>
          ...
          <div className="flex items-center gap-6">
            ...
            <button
              ...
              className="text-[10px] font-sans font-black uppercase tracking-[0.2em] text-muted hover:text-foreground"
            >
              Sign Out
            </button>
          </div>
      ...
      <div className="md:hidden fixed bottom-12 left-6 right-6 h-16 bg-foreground rounded-full flex justify-around items-center z-[60] shadow-2xl shadow-foreground/20">
        {[
          { id: "dashboard", icon: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" /> },
          { id: "invoices", icon: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /> },
          { id: "calendar", icon: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2-2v12a2 2 0 002 2z" /> },
        ].map(item => (
          <button
            key={item.id}
            onClick={() => { setView(item.id as any); setIsMoreMenuOpen(false); }}
            className={`transition-all ${view === item.id ? "text-background scale-110" : "text-background/40 hover:text-background/70"}`}
          >
        ...
        <button
          ...
          className={`transition-all ${isMoreMenuOpen ? "text-background scale-110" : "text-background/40 hover:text-background/70"}`}
        >
      ...
      {isMoreMenuOpen && (
        <div className="fixed inset-0 bg-background/95 backdrop-blur-xl z-[55] md:hidden flex flex-col p-12 text-center animate-in fade-in zoom-in-95 duration-200">
          <button
            onClick={() => setIsMoreMenuOpen(false)}
            className="absolute top-12 right-12 text-muted hover:text-foreground"
          >
          ...
          <div className="flex-1 flex flex-col justify-center gap-12">
            {[
              ...
              <button
                key={item.id}
                onClick={() => { setView(item.id as any); setIsMoreMenuOpen(false); }}
                className={`text-4xl font-serif tracking-tighter transition-all ${view === item.id ? "text-foreground" : "text-muted hover:text-foreground"
                  }`}
              >
                {item.label}
              </button>
          ...
          <div className="pb-24">
            <button
              onClick={() => db.auth.signOut()}
              className="text-[10px] font-sans font-black uppercase tracking-[0.2em] text-status-overdue"
            >
              Log Out
            </button>
          </div>
